...
- `phpcs.xml` enforces `WordPress` ruleset + `WordPress.Files.FileName` on `elements/` and `plugin.php` only.
- `plugin.php` defines constants (`HBE_PLUGIN_FILE`, `HBE_PLUGIN_DIR`, `HBE_PLUGIN_URL`, `HBE_VERSION`), requires all `includes/class-hbe-*.php`, then calls `HBE_Plugin::boot()`.
...
- `HBE_Calendar_Settings` — per-calendar settings (availability, services, mail, SMTP) via post meta
